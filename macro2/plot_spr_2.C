...
void plot_spr_2()
{
	...
	// save path
	TString savepath = Form("./analysis/%d/custom/",study);
	...
	while( file=(TSystemFile*)next() )
	{
		...
		for(pid=211; pid<=2212; pid+=2001)
		{
			...
			TString bgS_func = "gaus(0)+pol0(3)";
			TString bgD_func = "gaus(0)+pol0(3)";
			TString sim_func = getFitFunctionSim(track,pid);
			TString dat_func = getFitFunctionSim(track,pid); //getFitFunctionData(track,pid);
			TF1 *sim_fit  = new TF1("sim_fit",sim_func);
			TF1 *data_fit = new TF1("data_fit",dat_func);
			TF1 *sim_fit_bg  = new TF1("sim_fit_bg",bgS_func);
			TF1 *data_fit_bg = new TF1("data_fit_bg",bgD_func);
