...
int main(int argc,char** argv)
{
	...
	G4String macro, events, geometry, radiator, physlist, outfile, 
		session,geomAng,batchmode,lensId,particle,momentum,
		testVal1,testVal2, prismStepX,prismStepY,beamZ,beamX,
	...
	for ( G4int i=1; i<argc; i=i+2 ) {
		...
		else if ( G4String(argv[i]) == "-shift") shift = atof(argv[i+1]);
		else {
			PrintUsage();
			return 1;
		}
