				...
				double &mean, double &con)
{
	...
	// fit hist
	fit->SetParameters(con,mean,0.01,con/3);
...
void dataBGsub( Int_t studyID=151 )
{
	...
	while( file=(TSystemFile*)next() )
	{
		...
		TF1 *gausD = new TF1("gausD","gaus+pol0(3)");
		TF1 *gausS = new TF1("gausS","gaus+pol0(3)");
		...
		SpecSearch(spec, hDSim, gausS, meanS, con);
		SpecSearch(spec, hDDat, gausD, meanD, con);
		...
		grANGsim->SetPoint(n,track,tangS-meanS);
		grANGdat->SetPoint(n,track,tangD-meanD);
		...
		hDSim->Draw();
		c_tmp->Print(Form("../studies/bgsub/sim_bgsub_%.2f.png",track));
		...
		hDDat->Draw();
		c_tmp->Print(Form("../studies/bgsub/data_bgsub_%.2f.png",track));
	...
	leg->Draw();
	c1->Print("../studies/bgsub/angle_vs_SPR_bgsub.png");
	...
	leg->Draw();
	c2->Print("../studies/bgsub/angle_vs_thetaCdiff_bgsub.png");
	c2->Print("../studies/bgsub/C/angle_vs_thetaCdiff.C");
}
